...
public final class NekoRecipeNamespaces {
    ...
    private static void initialize() {
        NekoJSPluginManager.getPlugins().forEach(plugin -> plugin.registerRecipeNamespaces(NekoRecipeNamespaces::registerBridge));
        initialized = true;
    }

    @SuppressWarnings("unchecked")
    private static void registerBridge(String namespace, Object factory) {
        if (!(factory instanceof Function)) {
            throw new IllegalArgumentException("Factory for namespace '" + namespace + "' must be a Function<RecipeEventJS, Object>");
        }
        register(namespace, (Function<RecipeEventJS, Object>) factory);
    }
}