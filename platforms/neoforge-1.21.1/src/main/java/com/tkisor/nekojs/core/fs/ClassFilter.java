...
public class ClassFilter implements Predicate<String> {
     ...
     * 查询某个全限定类名是否在沙盒允许范围内。
     *
     * @param className 全限定类名，如 {@code "net.minecraft.world.item.ItemStack"}
     * @return {@code true} 如果该类可以通过 {@code Java.type()} 访问
     */
    public static boolean isAllowed(String className) {
        return INSTANCE.test(className);
    }

    /**
     * 返回当前生效的所有黑名单前缀（不可变集合）。
     * <p>
     * 用于外部 mod 参考哪些范围的类完全不可用。
     * </p>
     */
    public static Set<String> getBlacklistedPrefixes() {
        return Collections.unmodifiableSet(GENERAL_BLACKLIST);
    }
